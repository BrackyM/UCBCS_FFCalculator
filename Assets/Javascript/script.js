...
var playerData, nflNews, allUserLeagues, firstTwoLeagues, league1, league2, usernameData, nflLiveScore, userRosterID1, userRosterID2, twoLeagueDatabase, matchupID;
...
.then(data => {
   ...
   $(scoreText).text("Scores and News for Week " + nflLiveScore.week.number + " of " + nflLiveScore.season.year);
   ...
   for (let i=0; i < nflLiveScore.events.length; i++){
      ...
      $(newsText).append("<h4><a class='button is-info' href=" + nflLiveScore.events[i].links[0].href + " target='_blank' > " + nflLiveScore.events[i].name + " </a> --- " + newDate + "</h4>");
   }
})
; 
...
.then(data => {
   ...
   $('#welcome-user').append("<img src='https://sleepercdn.com/avatars/" + usernameData.avatar + "' id=user-image >");
   ...
   return fetch("https://api.sleeper.app/v1/user/" + usernameData.user_id + "/leagues/nfl/2022");
})
.then(response => {
   return response.json();
})
.then(data => {
   ...
   $('#team1card-content').append("<h2 class=title is-4 has-text-centered>" + allUserLeagues[0].name + "</h2>");
   $('#league2card-content').append("<h2 class=title is-4 has-text-centered>" + allUserLeagues[1].name + "</h2>"); 
   ...
   var rosterPositions1 = allUserLeagues[0].roster_positions.filter(function (item){
      return item !== "BN"
   })
   var rosterPositions2 = allUserLeagues[1].roster_positions.filter(function (item){
      return item !== "BN"
   })
      ...
      for (let i=0; i < rosterPositions1.length; i++){
         $('#teamcard-positions').append("<ul><li class='is-size-5'>" + allUserLeagues[0].roster_positions[i] + "</li></ul>");
      };
      for (let i=0; i < rosterPositions2.length; i++){
         $('#teamcard-positions2').append("<ul><li class='is-size-5'>" + allUserLeagues[1].roster_positions[i] + "</li></ul>");
      };
...
.then(data => {
   ...
   .then(data => {
      ...
      for (let i=0; i < findRosterID1[0].starters_points.length; i++){
         $('#teamcard-players1points').append("<ul class='has-text-right has-text-info'><li class='is-size-5'>"+ findRosterID1[0].starters_points[i] + "</ul></li>");
      }
      ...
      for (let i=0; i < findRosterID2[0].starters_points.length; i++){
         $('#team2card-players1points').append("<ul class='has-text-right has-text-info'><li class='is-size-5'>"+ findRosterID2[0].starters_points[i] + "</ul></li>");
      }
      ...
      var compareMatchingID1 = twoLeagueDatabase[0].filter( (twoLeagueDatabase) => {
      ...
      var compareMatchingID2 = twoLeagueDatabase[1].filter( (twoLeagueDatabase) => {
         ...
         for (let i=0; i < opponentRoster1[0].starters_points.length; i++){
            ...
            $('#teamcard-players2points').append("<ul class='has-text-left has-text-info'><li class='is-size-5'>"+ opponentRoster1[0].starters_points[i] + "</ul></li>");
         }
         // Looping through the points for our opponents' starting players in the second league
         for (let i=0; i < opponentRoster2[0].starters_points.length; i++){
            // populating their points within the points column on the index file
            $('#team2card-players2points').append("<ul class='has-text-left has-text-info'><li class='is-size-5'>"+ opponentRoster2[0].starters_points[i] + "</ul></li>");
         }
      ...
      console.log(opponentStarters2);
         ...
         var opponentStartersArray1 = []
            ...
            $('#teamcard-players2').append("<ul class='has-text-right'><li class='is-size-5'>"+ opponentStartersArray1[i].full_name + "</ul></li>")
            } else {
            // Else statement checks for the Defense positions of a Fantasy team. (defense first and last name are logged differently)
            $('#teamcard-players2').append("<ul class='has-text-right'><li class='is-size-5'>"+ opponentStartersArray1[i].last_name + "</ul></li>")
            }
            }
         ...
         var opponentStartersArray2 = []
            ...
            $('#team2card-players2').append("<ul class='has-text-right'><li class='is-size-5'>"+ opponentStartersArray2[i].full_name + "</ul></li>")
            } else {
            // Else statement checks for the Defense positions of a Fantasy team. (defense first and last name are logged differently)
            $('#team2card-players2').append("<ul class='has-text-right'><li class='is-size-5'>"+ opponentStartersArray2[i].last_name + "</ul></li>")
            }
            }
